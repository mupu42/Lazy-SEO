...
function lazy_seo_set_title ( $title ) {
  ...
  if ( function_exists( 'lazy_seo_get_titles_woo' ) && function_exists( 'is_product' ) ) {  
      if( is_product() ){   
        $titles = lazy_seo_get_titles_woo();       
        $firstkw = lazy_seo_woo_title();
        $firstgeo = "";
      } else {
...
function lazy_seo_h1( $content) {  
  ...
  //Only runs if WooCommerce extention is not installed  
  if (!function_exists( 'lazy_seo_woo_check' ) ){      
...
function lazy_seo_add_custom_box() {
    ...
    foreach ($screens as $screen) {
        ...
        'lazy_seo_inner_custom_box_post', // callback function, spits out the content
        'post', 
...
// Prints the box content for post and products
function lazy_seo_inner_custom_box_post() {
  global $post;
  // Use nonce for verification
  wp_nonce_field( plugin_basename( __FILE__ ), 'lazy_seo_noncename' );
...
<?php if (!function_exists( 'lazy_seo_woo_check' ) ){ ?>  
  <label for="lazy_meta_kw">Page Focus Keyword</label>  
  <select name="lazy_meta_kw">
  <?php for($k = 0; $k < count($kws); $k++){?>
    <option value="<?php echo esc_attr($kws[$k]); ?>"><?php echo esc_attr($kws[$k]); ?></option>
...
function lazy_seo_save_postdata( $post_id ) {
  ...
  if( function_exists( 'lazy_seo_woo_title' ) && function_exists( 'is_product' ) ) {
    if( is_product() ) {
      $kw = lazy_seo_woo_title( $post_id );
      $geo = "";
    }
  }
